...
struct Vect2 {
  ...
  Vect2(double setMag, double setTheta) : mag(setMag), theta(setTheta) {}
};
...
Eigen::Matrix4d transform3D(double yaw, double pitch, double roll, double xt,
                            double yt, double zt);
...
void renderRay(pcl::visualization::PCLVisualizer::Ptr &viewer, Point p1,
               Point p2, std::string name, Color color);
void renderPath(pcl::visualization::PCLVisualizer::Ptr &viewer,
                const PointCloudT::Ptr &cloud, std::string name, Color color);
...
struct LineSegment {
  ...
  LineSegment(float setMy, double setMx, double setB, double setMin,
              double setMax)
  ...
  LineSegment() : my(1), mx(1), b(1), min(-1), max(0) {}
  //			p1
  //			o
  //		p2		  p3
  // 		o---------o
  ...
  bool Intersect(LineSegment line, Point &intersection) {
    ...
    // not dealing with any vertical or horizontal lines and lines are not the
    // same slope
    else {
...
struct Lidar {
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr scan(vector<LineSegment> walls) {
    ...
    for (double angle = theta; angle < theta + 2 * pi - residue;
         angle += deltaTheta) {
      ...
      for (LineSegment wall : walls) {
        ...
        if (ray.Intersect(wall, point)) {
          ...
          // ray.Print();
          // wall.Print();
          double distance = sqrt((x - point.x) * (x - point.x) +
                                 (y - point.y) * (y - point.y));
