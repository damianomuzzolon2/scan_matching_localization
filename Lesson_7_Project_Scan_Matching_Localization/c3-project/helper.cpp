...
Pose getPose(Eigen::Matrix4d matrix) {
  ...
  Pose pose(Point(matrix(0, 3), matrix(1, 3), matrix(2, 3)),
            Rotate(atan2(matrix(1, 0), matrix(0, 0)),
...
void renderRay(pcl::visualization::PCLVisualizer::Ptr &viewer, Point p1,
  ...
  viewer->addLine(PointT(p1.x, p1.y, 0), PointT(p2.x, p2.y, 0), color.r,
...
void renderPath(pcl::visualization::PCLVisualizer::Ptr &viewer,
  ...
  for (int index = previous + 1; index < cloud->points.size(); index++) {
    renderRay(viewer,
              Point(cloud->points[previous].x, cloud->points[previous].y, 0),
              Point(cloud->points[index].x, cloud->points[index].y, 0),
              name + to_string(previous), color);
    previous++;
  }
}
...
Eigen::Quaternionf getQuaternion(float theta) {
  ...
  rotation_mat << cos(theta), -sin(theta), 0, sin(theta), cos(theta), 0, 0, 0,
      1;
...
void renderBox(pcl::visualization::PCLVisualizer::Ptr &viewer, BoxQ box, int id,
  ...
  viewer->addCube(box.bboxTransform, box.bboxQuaternion, box.cube_length,
                  box.cube_width, box.cube_height, cubeFill);
  viewer->setShapeRenderingProperties(
      pcl::visualization::PCL_VISUALIZER_REPRESENTATION,
